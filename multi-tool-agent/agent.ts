...
function mockProductInfo(name: string): z.infer<typeof ProductInfoOutput> {
  ...
  async ({ name }) => {
    ...
    if (!parsed.success) {
      return {
        isError: true,
        content: [
          { type: 'text' as const, text: `get_product_info produced invalid output: ${formatZodError(parsed.error)}` },
        ],
      };
    }
...
function mockOrderStatus(orderId: string): z.infer<typeof OrderStatusOutput> {
  ...
  async ({ orderId }) => {
    ...
    if (!parsed.success) {
      return {
        isError: true,
        content: [
          { type: 'text' as const, text: `get_order_status produced invalid output: ${formatZodError(parsed.error)}` },
        ],
      };
    }
    return { content: [{ type: 'text' as const, text: JSON.stringify(parsed.data) }] };
  ...
  async ({ price, percent }) => {
    ...
    if (!parsed.success) {
      return {
        isError: true,
        content: [
          { type: 'text' as const, text: `calculate_discount produced invalid output: ${formatZodError(parsed.error)}` },
        ],
      };
    }
    return { content: [{ type: 'text' as const, text: JSON.stringify(parsed.data) }] };
...
async function* readUserMessages(): AsyncGenerator<SDKUserMessage> {
  while (true) {
