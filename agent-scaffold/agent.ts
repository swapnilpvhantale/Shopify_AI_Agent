...
// Readonly<Record<string, number>> = "an object of string keys -> number
// values that TypeScript won't let you mutate after creation."
const MOCK_STOCK_LEVELS: Readonly<Record<string, number>> = {
  'WB-1L': 42,
};
...
const getStockLevel = tool(
  ...
  async ({ sku }) => {
    ...
    return {
      content: [{ type: 'text' as const, text: `SKU ${sku}: ${count} in stock` }],
    };
  },
);
...
const approveToolUse: CanUseTool = async (toolName, input, { title }) => {
  console.log('\n--- permission request ---');
  ...
  // PermissionResult must be one of these two exact shapes: 'allow' (with
  // the input to actually pass through) or 'deny' (with a reason message).
